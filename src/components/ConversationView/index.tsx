...
const ConversationView = () => {
  ...
  // 清理 SQL 语句，移除多余的分号
  const cleanSqlStatement = (sql: string): string => {
    // 移除字符串中的多余空格
    ...
    // 如果 SQL 中包含多个语句（由分号分隔），只保留第一个语句
    if (cleanedSql.includes(';')) {
      cleanedSql = cleanedSql.split(';')[0].trim();
    }
    ...
    if (limitMatch) {
      // 移除原有的 LIMIT
      cleanedSql = cleanedSql.replace(/LIMIT\s+\d+/i, '').trim();
      // 在最后添加 LIMIT
      cleanedSql += ' ' + limitMatch[0];
    }
   ...
   - 如果遇到语法错误，提供修正建议`;
    ...
    try {
      const enhanceResponse = await fetch("/api/enhance-prompt", {
        ...
        body: JSON.stringify({
          question: userPrompt,
          basePrompt: sqlContextPrompt,
          connection: {
            id: currentConnection?.id || "default",
            title: currentConnection?.title || "MySQL Connection",
            engineType: currentConnection?.engineType || "MYSQL",
            host: currentConnection?.host || "localhost",
            port: currentConnection?.port || "3306",
            username: currentConnection?.username || "root",
            password: currentConnection?.password || "",
            database: currentDatabase?.name || currentConnection?.database || ""
          }
        }),
      });

      if (!enhanceResponse.ok) {
        throw new Error(`Failed to enhance prompt: ${enhanceResponse.statusText}`);
      }

      const { enhancedPrompt } = await enhanceResponse.json();
      console.log('[ConversationView] Enhanced prompt:', enhancedPrompt);
      ...
      if (settingStore.setting.activeProvider === "qwen") {
        // Internal Qwen - First enhance prompt with schema information
        ...
        const { enhancedPrompt: enhancedPromptWithSchema } = await enhanceRes.json();
        
        messages = [
          {
            role: "system",
            content: enhancedPromptWithSchema
          },
          ...baseMessages,
        ];
      ...
      const rawRes = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          messages,
          connection: settingStore.setting.activeProvider === "qwen" ? {
            id: currentConnection?.id || "default",
            title: currentConnection?.title || "MySQL Connection",
            engineType: currentConnection?.engineType || "MYSQL",
            host: currentConnection?.host || "localhost",
        ...
        if (settingStore.setting.activeProvider === "qwen") {
          ...
          if (data.message) {
            ...
            if (aiResponse.content) {
              // 使用正则表达式提取 SQL 语句
              const sqlMatch = aiResponse.content.match(/```sql\n([\s\S]*?)\n```/);
              if (sqlMatch) {
                const originalSql = sqlMatch[1];
                const cleanedSql = cleanSqlStatement(originalSql);
                // 替换原始 SQL 为清理后的 SQL
                aiResponse.content = aiResponse.content.replace(originalSql, cleanedSql);
              }
            }
            messageStore.updateMessage(assistantMessage.id, {
